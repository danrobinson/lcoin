...
function SOCKS() {
  if (!(this instanceof SOCKS)) return new SOCKS();
...
SOCKS.errors = ['', 'General failure', 'Connection not allowed', 'Network is unreachable', 'Host is unreachable', 'Connection refused', 'TTL expired', 'Command not supported', 'Address type not supported', 'Unknown proxy error'];
...
SOCKS.prototype.getError = function getError(code) {
  if (code >= SOCKS.errors.length) return SOCKS.errors[9];
...
SOCKS.prototype.open = function open(options) {
  ...
  if (options.username != null) {
    ...
    assert(typeof options.password === 'string', 'Username must have a password.');
  }
...
SOCKS.prototype.sendProxy = function sendProxy() {
  ...
  if (type === 0x03) packet.writeU8(name.length);
...
function Proxy(host, port, user, pass) {
  if (!(this instanceof Proxy)) return new Proxy(host, port, user, pass);
...
Proxy.prototype.connect = co( /*#__PURE__*/_regenerator2.default.mark(function connect(port, host) {
  var self, i, options, socket;
  return _regenerator2.default.wrap(function connect$(_context) {
    while (1) {
...
function parseAddr(data, offset) {
  ...
  if (br.left() < offset + 2) throw new Error('Bad SOCKS address length.');
  ...
  switch (type) {
    case 0x01:
      if (br.left() < 6) throw new Error('Bad SOCKS ipv4 length.');
    ...
    case 0x03:
      ...
      if (br.left() < len + 2) throw new Error('Bad SOCKS domain length.');

      host = br.readString(len, 'utf8');
      port = br.readU16BE();
      break;
    case 0x04:
      if (br.left() < 18) throw new Error('Bad SOCKS ipv6 length.');
