...
function MemDB(location) {
  if (!(this instanceof MemDB)) return new MemDB(location);
...
MemDB.prototype.range = function range(min, max) {
  if (typeof min === 'string') min = Buffer.from(min, 'utf8');

  if (typeof max === 'string') max = Buffer.from(max, 'utf8');
...
MemDB.prototype.get = function get(key, options, callback) {
  ...
  if (options.asBuffer === false) value = value.toString('utf8');
  ...
  if (this.options.reverse) {
    if (this.options.end) {
      ...
      if (this.options.lt && iter.valid()) {
        if (iter.compare(this.options.end) === 0) iter.prev();
      }
    } else {
      iter.seekLast();
    }
    ...
    if (this.options.start) {
      ...
      if (this.options.gt && iter.valid()) {
        if (iter.compare(this.options.start) === 0) iter.next();
      }
    } else {
      iter.seekFirst();
    }
...
Iterator.prototype.next = function (callback) {
  ...
  if (options.reverse) {
    ...
    if (result && options.start) {
      if (options.gt) {
        if (iter.compare(options.start) <= 0) result = false;
      } else {
        if (iter.compare(options.start) < 0) result = false;
      }
    }
    ...
    if (result && options.end) {
      if (options.lt) {
        if (iter.compare(options.end) >= 0) result = false;
      } else {
        if (iter.compare(options.end) > 0) result = false;
      }
    }
  ...
  if (!options.keys) key = DUMMY;

  if (!options.values) value = DUMMY;

  if (!options.keyAsBuffer) key = key.toString('utf8');

  if (!options.valueAsBuffer) value = value.toString('utf8');
...
Iterator.prototype.seek = function seek(key) {
  ...
  if (this.options.reverse) this.iter.seekMax(key);else this.iter.seekMin(key);
};
...
IteratorOptions.prototype.fromOptions = function fromOptions(options) {
  ...
  if (options.start != null) this.start = options.start;

  if (options.end != null) this.end = options.end;

  if (options.gte != null) this.start = options.gte;

  if (options.lte != null) this.end = options.lte;
  ...
  if (this.start != null) {
    if (typeof this.start === 'string') this.start = Buffer.from(this.start, 'utf8');
    assert(Buffer.isBuffer(this.start), '`start` must be a Buffer.');
  }

  if (this.end != null) {
    if (typeof this.end === 'string') this.end = Buffer.from(this.end, 'utf8');
