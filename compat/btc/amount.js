...
function Amount(value, unit, num) {
  if (!(this instanceof Amount)) return new Amount(value, unit, num);

  this.value = 0;

  if (value != null) this.fromOptions(value, unit, num);
}
...
Amount.prototype.fromOptions = function fromOptions(value, unit, num) {
  if (typeof unit === 'string') return this.from(unit, value, num);
...
Amount.btc = function btc(value, num) {
  if (util.isFloat(value)) return value;
...
Amount.value = function _value(value, num) {
  if (util.isInt(value)) return value;
...
Amount.parse = function parse(value, exp, num) {
  ...
  assert(hi < maxHi || hi === maxHi && lo <= maxLo, 'Number exceeds 2^53-1.');
