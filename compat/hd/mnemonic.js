...
function Mnemonic(options) {
  if (!(this instanceof Mnemonic)) return new Mnemonic(options);
...
Mnemonic.prototype.toSeed = function toSeed(passphrase) {
  ...
  return crypto.pbkdf2(Buffer.from(phrase, 'utf8'), Buffer.from(passwd, 'utf8'), 2048, 64, 'sha512');
};
...
Mnemonic.prototype.getEntropy = function getEntropy() {
  if (!this.entropy) this.entropy = crypto.randomBytes(this.bits / 8);
...
Mnemonic.prototype.fromPhrase = function fromPhrase(phrase) {
  ...
  for (i = 0; i < words.length; i++) {
    ...
    if (index === -1) throw new Error('Could not find word.');
  ...
  for (i = 0; i < cbits; i++) {
    ...
    b = ent[oct] >>> 7 - bit & 1;
    j = chk[oct] >>> 7 - bit & 1;
    if (b !== j) throw new Error('Invalid checksum.');
  }
...
Mnemonic.getLanguage = function getLanguage(word) {
  ...
  for (i = 0; i < Mnemonic.languages.length; i++) {
    ...
    wordlist = Mnemonic.getWordlist(lang);
    if (util.binarySearch(wordlist, word, util.strcmp) !== -1) return lang;
  }
...
Mnemonic.isMnemonic = function isMnemonic(obj) {
  return obj && typeof obj.bits === 'number' && typeof obj.toSeed === 'function';
};
