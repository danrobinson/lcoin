...
common.parsePath = function parsePath(path, max) {
  ...
  if (max == null) max = common.MAX_INDEX;
  ...
  for (i = 0; i < parts.length; i++) {
    ...
    if (!/^\d+$/.test(index)) throw new Error('Non-number path index.');

    index = parseInt(index, 10);

    if (hardened) index += common.HARDENED;

    if (!(index >= 0 && index < max)) throw new Error('Index out of range.');
...
common.isMaster = function isMaster(key) {
  return key.depth === 0 && key.childIndex === 0 && key.parentFingerPrint.readUInt32LE(0, true) === 0;
};
...
common.isAccount44 = function isAccount44(key, accountIndex) {
  if (accountIndex != null) {
    if (key.childIndex !== common.HARDENED + accountIndex) return false;
  }
