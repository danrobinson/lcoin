...
function HDPrivateKey(options) {
  if (!(this instanceof HDPrivateKey)) return new HDPrivateKey(options);
...
HDPrivateKey.prototype.fromOptions = function fromOptions(options) {
  ...
  if (options.network) this.network = Network.get(options.network);
...
HDPrivateKey.prototype.xprivkey = function xprivkey() {
  if (!this._xprivkey) this._xprivkey = this.toBase58();
...
HDPrivateKey.prototype.derive = function derive(index, hardened) {
  ...
  if (hardened && index < common.HARDENED) index += common.HARDENED;

  if (index < 0 || index >= common.MAX_INDEX) throw new Error('Index out of range.');
  ...
  if (!this.fingerPrint) this.fingerPrint = crypto.hash160(this.publicKey).slice(0, 4);
...
HDPrivateKey.prototype.getID = function getID(index) {
  return this.network.keyPrefix.xprivkey58 + this.publicKey.toString('hex') + index;
};
...
HDPrivateKey.prototype.deriveAccount44 = function deriveAccount44(accountIndex) {
  ...
  return this.derive(44, true).derive(this.network.keyPrefix.coinType, true).derive(accountIndex, true);
};
...
HDPrivateKey.prototype.equal = function equal(obj) {
  if (!HDPrivateKey.isHDPrivateKey(obj)) return false;

  return this.network === obj.network && this.depth === obj.depth && this.parentFingerPrint.equals(obj.parentFingerPrint) && this.childIndex === obj.childIndex && this.chainCode.equals(obj.chainCode) && this.privateKey.equals(obj.privateKey);
};
...
HDPrivateKey.prototype.fromSeed = function fromSeed(seed, network) {
  ...
  if (!ec.privateKeyVerify(left)) throw new Error('Master private key is invalid.');
...
HDPrivateKey.isHDPrivateKey = function isHDPrivateKey(obj) {
  return obj && typeof obj.derive === 'function' && typeof obj.fromMnemonic === 'function' && Buffer.isBuffer(obj.chainCode);
};
