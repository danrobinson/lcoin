...
function Bloom(size, n, tweak, update) {
  if (!(this instanceof Bloom)) return new Bloom(size, n, tweak, update);
...
Bloom.prototype.fromOptions = function fromOptions(size, n, tweak, update) {
  ...
  if (update == null || update === -1) update = Bloom.flags.NONE;
...
Bloom.fromRate = function fromRate(items, rate, update) {
  ...
  if (update !== -1) {
    assert(size <= Bloom.MAX_BLOOM_FILTER_SIZE * 8, 'Bloom filter size violates policy limits!');
  }

  n = Math.max(1, size / items * LN2 | 0);

  if (update !== -1) {
    assert(n <= Bloom.MAX_HASH_FUNCS, 'Bloom filter size violates policy limits!');
  }
...
Bloom.prototype.isWithinConstraints = function isWithinConstraints() {
  if (this.size > Bloom.MAX_BLOOM_FILTER_SIZE * 8) return false;

  if (this.n > Bloom.MAX_HASH_FUNCS) return false;
