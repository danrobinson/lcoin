...
encoding.read64BN = function read64BN(data, off) {
  ...
  if (num[num.length - 1] & 0x80) return new BN(num, 'le').notn(64).addn(1).neg();
...
encoding.read64BEBN = function read64BEBN(data, off) {
  ...
  if (num[0] & 0x80) return new BN(num, 'be').notn(64).addn(1).neg();
...
encoding._write64BN = function _write64BN(dst, num, off, be) {
  ...
  if (bits <= 53) return encoding._write64(dst, num.toNumber(), off, be);

  if (bits > 64) num = num.maskn(64);

  if (num.isNeg()) num = num.neg().inotn(64).iaddn(1);
...
encoding.readVarint2BN = function readVarint2BN(data, off) {
  ...
  while (num < 0x3fffffffffff) {
    ...
    if ((ch & 0x80) === 0) return new Varint(size, new BN(num));
...
encoding.writeVarint2BN = function writeVarint2BN(dst, num, off) {
  ...
  if (num.bitLength() <= 53) return encoding.writeVarint2(dst, num.toNumber());

  for (;;) {
    tmp[len] = num.words[0] & 0x7f | (len ? 0x80 : 0x00);
    if (num.cmpn(0x7f) <= 0) break;
...
encoding.sizeVarint2BN = function sizeVarint2BN(num) {
  ...
  if (num.bitLength() <= 53) return encoding.sizeVarint(num.toNumber());
...
function enforce(value, offset, reason) {
  if (!value) throw new encoding.EncodingError(offset, reason);
}

function assert(value, offset) {
  if (!value) throw new encoding.EncodingError(offset, 'Out of bounds read');
}