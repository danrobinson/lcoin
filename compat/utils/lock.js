...
Lock.prototype.lock = function lock(arg1, arg2) {
  ...
  if (this.busy) {
    if (name) {
      if (!this.map[name]) this.map[name] = 0;
      this.map[name]++;
    }
...
Lock.prototype.unlock = function unlock() {
  ...
  if (job.name) {
    ...
    if (--this.map[job.name] === 0) delete this.map[job.name];
  }
...
function MappedLock() {
  ...
  this.jobs = (0, _create2.default)(null);
  this.busy = (0, _create2.default)(null);
  this.destroyed = false;
}
...
MappedLock.prototype.lock = function lock(key, force) {
  ...
  if (this.busy[key]) {
    return new _promise2.default(function (resolve, reject) {
      if (!self.jobs[key]) self.jobs[key] = [];
...
MappedLock.prototype.unlock = function unlock(key) {
  ...
  return function unlocker() {
    ...
    if (jobs.length === 0) delete self.jobs[key];
