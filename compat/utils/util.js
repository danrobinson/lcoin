...
if (os.homedir) util.HOME = os.homedir();
...
util.isHex = function isHex(obj) {
  return typeof obj === 'string' && /^[0-9a-f]+$/i.test(obj) && obj.length % 2 === 0;
};
...
if (_assign2.default) util.merge = _assign2.default;
...
util.isNumber = function isNumber(value) {
  return typeof value === 'number' && isFinite(value) && util.isSafeInteger(value);
};
...
util.isFloat = function isFloat(value) {
  return typeof value === 'string' && /^-?(\d+)?(?:\.\d*)?$/.test(value) && value.length !== 0 && value !== '-';
};
...
util.inspectify = function inspectify(obj, color) {
  return typeof obj !== 'string' ? nodeUtil.inspect(obj, null, 20, color !== false) : obj;
};
...
util.format = function format(args, color) {
  if (color == null) color = process.stdout ? process.stdout.isTTY : false;

  return (0, _typeof3.default)(args[0]) === 'object' ? util.inspectify(args[0], color) : nodeUtil.format.apply(nodeUtil, args);
};
...
util.log = function log() {
  ...
  for (i = 0; i < args.length; i++) {
    ...
    msg = (0, _typeof3.default)(args[0]) !== 'object' ? util.format(args, false) : args[0];
    console.log(msg);
    return;
  }
...
util.inherits = function inherits(child, parent) {
  ...
  if (_setPrototypeOf2.default) {
    (0, _setPrototypeOf2.default)(child.prototype, parent.prototype);
