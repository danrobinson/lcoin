...
function RBT(compare, unique) {
  if (!(this instanceof RBT)) return new RBT(compare, unique);
...
RBT.prototype.search = function search(key) {
  ...
  while (!current.isNull()) {
    ...
    if (cmp < 0) current = current.left;else current = current.right;
  }
};
...
RBT.prototype.insert = function insert(key, value) {
  ...
  if (left) parent.left = node;else parent.right = node;
...
RBT.prototype.removeNode = function removeNode(z) {
  ...
  if (!z.left.isNull() && !z.right.isNull()) y = this.successor(z);
  ...
  if (y.color === BLACK) this.removeFixup(x);
};
...
RBT.prototype.rotl = function rotl(x) {
  ...
  if (!y.left.isNull()) y.left.parent = x;
...
RBT.prototype.rotr = function rotr(x) {
  ...
  if (!y.right.isNull()) y.right.parent = x;
...
RBT.prototype.clone = function clone() {
  ...
  for (;;) {
    if (!current.isNull()) {
      ...
      if (left) {
        if (parent) parent.left = copy;else snapshot = copy;
      } else {
        if (parent) parent.right = copy;else snapshot = copy;
      }
...
RBT.prototype.snapshot = function snapshot() {
  ...
  if (this.root.isNull()) return node;
...
RBT.prototype.range = function range(min, max) {
  ...
  if (min) iter.seekMin(min);else iter.seekFirst();

  while (iter.next()) {
    if (max && iter.compare(max) > 0) break;
