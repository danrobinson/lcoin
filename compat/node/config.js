...
Config.prototype.num = function num(key, fallback) {
  ...
  if (typeof value !== 'string') {
    if (typeof value !== 'number') throw new Error(key + ' must be a positive integer.');
    return value;
  }
  ...
  if (!isFinite(value)) throw new Error(key + ' must be a positive integer.');
...
Config.prototype.flt = function flt(key, fallback) {
  ...
  if (typeof value !== 'string') {
    if (typeof value !== 'number') throw new Error(key + ' must be a float.');
    return value;
  }

  if (!/^\d*(?:\.\d*)?$/.test(value)) throw new Error(key + ' must be a float.');

  value = parseFloat(value);

  if (!isFinite(value)) throw new Error(key + ' must be a float.');
...
Config.prototype.amt = function amt(key, fallback) {
  ...
  if (typeof value !== 'string') {
    if (typeof value !== 'number') throw new Error(key + ' must be an amount.');
    return value;
  }

  if (!/^\d+(\.\d{0,8})?$/.test(value)) throw new Error(key + ' must be an amount.');

  value = parseFloat(value);

  if (!isFinite(value)) throw new Error(key + ' must be an amount.');

  value *= 1e8;

  if (value % 1 !== 0 || value < 0 || value > 0x1fffffffffffff) throw new Error(key + ' must be an amount (uint64).');
...
Config.prototype.buf = function buf(key, fallback) {
  ...
  if (typeof value !== 'string') {
    if (!Buffer.isBuffer(value)) throw new Error(key + ' must be a buffer.');
    return value;
  }

  data = Buffer.from(value, 'hex');

  if (data.length !== value.length / 2) throw new Error(key + ' must be a hex string.');
...
Config.prototype.obj = function obj(key, fallback) {
  ...
  if (!value || (typeof value === 'undefined' ? 'undefined' : (0, _typeof3.default)(value)) !== 'object') throw new Error(key + ' must be an object.');
...
Config.prototype.func = function func(key, fallback) {
  ...
  if (!value || typeof value !== 'function') throw new Error(key + ' must be a function.');
...
Config.prototype.getPrefix = function getPrefix() {
  ...
  if (prefix) {
    if (prefix[0] === '~') prefix = util.HOME + prefix.substring(1);
    return prefix;
  }
  ...
  if (network) {
    assert(isAlpha(network), 'Bad network.');
    if (network !== 'main') prefix += '/' + network;
  }
...
Config.prototype.parseForm = function parseForm(query, map) {
  ...
  if (query[0] === '?' || query[0] === '#') query = query.substring(1);
