...
function ConfirmStats(type, logger) {
  if (!(this instanceof ConfirmStats)) return new ConfirmStats(type, logger);
  ...
  if (logger) {
    assert((typeof logger === 'undefined' ? 'undefined' : (0, _typeof3.default)(logger)) === 'object');
...
ConfirmStats.prototype.fromRaw = function fromRaw(data) {
  ...
  }if (decay <= 0 || decay >= 1) throw new Error('Decay must be between 0 and 1 (non-inclusive).');

  if (buckets.length <= 1 || buckets.length > 1000) throw new Error('Must have between 2 and 1000 fee/pri buckets.');

  if (avg.length !== buckets.length) throw new Error('Mismatch in fee/pri average bucket count.');

  if (txAvg.length !== buckets.length) throw new Error('Mismatch in tx count bucket count.');

  if (maxConfirms <= 0 || maxConfirms > 6 * 24 * 7) throw new Error('Must maintain estimates for between 1 and 1008 confirms.');

  for (i = 0; i < maxConfirms; i++) {
    if (confAvg[i].length !== buckets.length) throw new Error('Mismatch in fee/pri conf average bucket count.');
  }
...
function PolicyEstimator(logger) {
  if (!(this instanceof PolicyEstimator)) return new PolicyEstimator(logger);
  ...
  if (policy.FREE_THRESHOLD >= MIN_PRIORITY) this.minTrackedPri = policy.FREE_THRESHOLD;
...
PolicyEstimator.prototype.init = function init() {
  ...
  for (boundary = this.minTrackedFee; boundary <= MAX_FEERATE; boundary *= FEE_SPACING) {
    fee.push(boundary);
  }

  fee.push(INF_FEERATE);

  for (boundary = this.minTrackedPri; boundary <= MAX_PRIORITY; boundary *= PRI_SPACING) {
...
PolicyEstimator.prototype.isFeePoint = function isFeePoint(fee, priority) {
  if (priority < this.minTrackedPri && fee >= this.minTrackedFee || priority < this.priUnlikely && fee > this.feeLikely) {
    return true;
  }
  return false;
};
...
PolicyEstimator.prototype.isPriPoint = function isPriPoint(fee, priority) {
  if (fee < this.minTrackedFee && priority >= this.minTrackedPri || fee < this.feeUnlikely && priority > this.priLikely) {
    return true;
  }
  return false;
};
...
PolicyEstimator.prototype.processBlockTX = function processBlockTX(height, entry) {
  ...
  if (blocks <= 0) {
    this.logger.debug('Block tx %s had negative blocks to confirm (%d, %d).', entry.txid(), height, entry.height);
    return;
  }
  ...
  if (fee === 0 || this.isPriPoint(rate, priority)) this.priStats.record(blocks, priority);else if (this.isFeePoint(rate, priority)) this.feeStats.record(blocks, rate);
};
...
PolicyEstimator.prototype.processBlock = function processBlock(height, entries, current) {
  ...
  this.logger.debug('Done updating estimates' + ' for %d confirmed entries. New mempool map size %d.', entries.length, this.map.size);
...
PolicyEstimator.prototype.fromRaw = function fromRaw(data) {
  ...
  if (br.readU8() !== PolicyEstimator.VERSION) throw new Error('Bad serialization version for estimator.');
...
function DoubleMap() {
  if (!(this instanceof DoubleMap)) return new DoubleMap();
