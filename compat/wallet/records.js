...
function ChainState() {
  if (!(this instanceof ChainState)) return new ChainState();
...
ChainState.prototype.fromRaw = function fromRaw(data) {
  ...
  if (br.left() > 0) this.marked = br.readU8() === 1;
...
function BlockMeta(hash, height, ts) {
  if (!(this instanceof BlockMeta)) return new BlockMeta(hash, height, ts);
...
function BlockMapRecord(height) {
  if (!(this instanceof BlockMapRecord)) return new BlockMapRecord(height);
...
function TXRecord(tx, block) {
  if (!(this instanceof TXRecord)) return new TXRecord(tx, block);
