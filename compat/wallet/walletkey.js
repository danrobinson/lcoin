...
function WalletKey(options, network) {
  if (!(this instanceof WalletKey)) return new WalletKey(options, network);
...
WalletKey.prototype.fromHD = function fromHD(account, key, branch, index) {
  ...
  if (key.privateKey) return this.fromPrivate(key.privateKey, account.network);
...
WalletKey.isWalletKey = function isWalletKey(obj) {
  return obj && Buffer.isBuffer(obj.publicKey) && typeof obj.index === 'number' && typeof obj.toPath === 'function';
};
