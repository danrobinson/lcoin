...
var layout = {
  ...
  T: function T(address, hash) {
    ...
    if (address.length === 64) return 'W' + address + hex(hash);
  ...
  C: function C(address, hash, index) {
    ...
    if (address.length === 64) return 'X' + address + hex(hash) + pad32(index);
  ...
  Tt: function Tt(key) {
    return key.length === 129 ? key.slice(64) : key.slice(41);
  }
};
