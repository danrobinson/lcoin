...
MTX.prototype.fromOptions = function fromOptions(options) {
  ...
  if (options.changeIndex != null) {
    if (options.changeIndex !== -1) {
      assert(util.isUInt32(options.changeIndex), 'Change index must be a uint32.');
...
MTX.prototype.scriptInput = function scriptInput(index, coin, ring) {
  ...
  if (prev.isScripthash()) {
    ...
    if (redeem.isProgram()) {
      ...
      if (redeem.isWitnessScripthash()) {
        ...
        if (!this.scriptVector(prev, input.witness, ring)) return false;
    ...
    // Regular P2SH.
    if (!this.scriptVector(redeem, input.script, ring)) return false;
  ...
  if (prev.isProgram()) {
    ...
    if (prev.isWitnessScripthash()) {
      ...
      if (!this.scriptVector(redeem, input.witness, ring)) return false;
...
MTX.prototype.scriptVector = function scriptVector(prev, vector, ring) {
  ...
  // P2PK
  if (prev.isPubkey()) {
    if (!prev.get(0).equals(ring.publicKey)) return false;
  ...
  if (prev.isPubkeyhash()) {
    if (!prev.get(2).equals(ring.getKeyHash())) return false;
  ...
  // Multisig
  if (prev.isMultisig()) {
    if (prev.indexOf(ring.publicKey) === -1) return false;
...
MTX.prototype.signVector = function signVector(prev, vector, sig, ring) {
  ...
  if (prev.isPubkey()) {
    ...
    if (!ring.publicKey.equals(prev.get(0))) return false;

    // Already signed.
    if (Script.isSignature(vector.get(0))) return true;

    if (vector.getSmall(0) !== 0) throw new Error('Input has not been templated.');
  ...
  if (prev.isPubkeyhash()) {
    ...
    if (!ring.getKeyHash().equals(prev.get(2))) return false;

    // Already signed.
    if (Script.isSignature(vector.get(0))) return true;

    if (!Script.isKey(vector.get(1))) throw new Error('Input has not been templated.');
  ...
  if (prev.isMultisig()) {
    ...
    if (vector.length - 1 > n) return false;

    // Count the number of current signatures.
    total = 0;
    for (i = 1; i < vector.length; i++) {
      if (Script.isSignature(vector.get(i))) total++;
    }

    // Signatures are already finalized.
    if (total === m && vector.length - 1 === m) return true;
    ...
    if (total >= m) {
      ...
      for (i = vector.length - 1; i >= 1; i--) {
        if (vector.getSmall(i) === 0) vector.remove(i);
      }
...
MTX.prototype.isVectorSigned = function isVectorSigned(prev, vector) {
  ...
  if (prev.isPubkey()) {
    if (!Script.isSignature(vector.get(0))) return false;
    return true;
  }
  ...
  if (prev.isMultisig()) {
    ...
    for (i = 1; i < vector.length; i++) {
      if (!Script.isSignature(vector.get(i))) return false;
    }
...
MTX.prototype.estimateSize = co( /*#__PURE__*/_regenerator2.default.mark(function estimateSize(estimate) {
  var scale, total, i, input, output, size, prev, coin;
  return _regenerator2.default.wrap(function estimateSize$(_context) {
    while (1) {
...
MTX.prototype.subtractFee = function subtractFee(fee, index) {
  ...
  if (typeof index === 'number') {
    ...
    if (output.value < min) throw new Error('Could not subtract fee.');
...
MTX.prototype.fund = co( /*#__PURE__*/_regenerator2.default.mark(function fund(coins, options) {
  var i, select, change;
  return _regenerator2.default.wrap(function fund$(_context2) {
    while (1) {
      switch (_context2.prev = _context2.next) {
        ...
        case 5:
          ...
          if (select.shouldSubtract) this.subtractFee(select.fee, select.subtractFee);
          ...
          return _context2.abrupt('return', select);
...
MTX.isMTX = function isMTX(obj) {
  return obj && Array.isArray(obj.inputs) && typeof obj.locktime === 'number' && typeof obj.scriptInput === 'function';
};
...
CoinSelector.prototype.total = function total() {
  if (this.shouldSubtract) return this.outputValue;
...
CoinSelector.prototype.select = co( /*#__PURE__*/_regenerator2.default.mark(function select(coins) {
  return _regenerator2.default.wrap(function select$(_context3) {
    while (1) {
...
CoinSelector.prototype.selectEstimate = co( /*#__PURE__*/_regenerator2.default.mark(function selectEstimate() {
  var change, size;
  return _regenerator2.default.wrap(function selectEstimate$(_context4) {
    while (1) {
