...
function NetAddress(options) {
  if (!(this instanceof NetAddress)) return new NetAddress(options);
...
NetAddress.DEFAULT_SERVICES = 0 | common.services.NETWORK | common.services.WITNESS | common.services.BLOOM;
...
NetAddress.prototype.inspect = function inspect() {
  return '<NetAddress:' + ' hostname=' + this.hostname + ' services=' + this.services.toString(2) + ' date=' + util.date(this.ts) + '>';
};
