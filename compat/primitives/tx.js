...
TX.prototype.hash = function _hash(enc) {
  ...
  if (!hash) {
    ...
    if (!this.mutable) this._hash = hash;
  }
...
TX.prototype.signatureHashV0 = function signatureHashV0(index, prev, type) {
  ...
  if (hashType === Script.hashType.SINGLE) {
    ...
    if (index >= this.outputs.length) return util.copy(encoding.ONE_HASH);
  }
...
TX.prototype.signatureHashV1 = function signatureHashV1(index, prev, value, type) {
  ...
  if (!(type & Script.hashType.ANYONECANPAY) && (type & 0x1f) !== Script.hashType.SINGLE && (type & 0x1f) !== Script.hashType.NONE) {
  ...
  if ((type & 0x1f) !== Script.hashType.SINGLE && (type & 0x1f) !== Script.hashType.NONE) {
...
TX.prototype.verify = function verify(view, flags) {
  ...
  for (i = 0; i < this.inputs.length; i++) {
    ...
    if (!this.verifyInput(i, coin, flags)) return false;
  }
...
TX.prototype.verifyAsync = co( /*#__PURE__*/_regenerator2.default.mark(function verifyAsync(view, flags) {
  return _regenerator2.default.wrap(function verifyAsync$(_context) {
    while (1) {
      switch (_context.prev = _context.next) {
        ...
        case 4:
          _context.next = 6;
          return workerPool.verify(this, view, flags);
...
TX.prototype.verifyInputAsync = co( /*#__PURE__*/_regenerator2.default.mark(function verifyInputAsync(index, coin, flags) {
  var input;
  return _regenerator2.default.wrap(function verifyInputAsync$(_context2) {
    while (1) {
      switch (_context2.prev = _context2.next) {
        case 0:
          ...
          assert(input, 'Input does not exist.');
          _context2.next = 4;
          return workerPool.verifyInput(this, index, coin, flags);
...
TX.prototype._getInputAddresses = function getInputAddresses(view) {
  ...
  if (this.isCoinbase()) return new AddrResult(addrs, table);
...
TX.prototype.getInputHashes = function getInputHashes(view, enc) {
  ...
  if (enc === 'hex') {
    ...
    return (0, _keys2.default)(input.table);
  }
...
TX.prototype.getOutputHashes = function getOutputHashes(enc) {
  ...
  if (enc === 'hex') {
    ...
    return (0, _keys2.default)(output.table);
  }
...
TX.prototype.getHashes = function getHashes(view, enc) {
  ...
  if (enc === 'hex') {
    ...
    return (0, _keys2.default)(result.table);
  }
...
TX.prototype.hasCoins = function hasCoins(view) {
  ...
  for (i = 0; i < this.inputs.length; i++) {
    input = this.inputs[i];
    if (!view.hasEntry(input)) return false;
  }
...
TX.prototype.isFinal = function isFinal(height, ts) {
  ...
  if (this.locktime < (this.locktime < THRESHOLD ? height : ts)) return true;
...
TX.prototype.verifyLocktime = function verifyLocktime(index, locktime) {
  ...
  if (!(this.locktime < THRESHOLD && locktime < THRESHOLD || this.locktime >= THRESHOLD && locktime >= THRESHOLD)) {
    return false;
  }

  if (locktime > this.locktime) return false;
...
TX.prototype.verifySequence = function verifySequence(index, locktime) {
  ...
  if ((locktime & DISABLE_FLAG) !== 0) return true;

  if (this.version < 2) return false;

  if ((input.sequence & DISABLE_FLAG) !== 0) return false;

  mask = TYPE_FLAG | SEQUENCE_MASK;
  sequence = input.sequence & mask;
  predicate = locktime & mask;

  if (!(sequence < TYPE_FLAG && predicate < TYPE_FLAG || sequence >= TYPE_FLAG && predicate >= TYPE_FLAG)) {
    return false;
  }

  if (predicate > sequence) return false;
...
TX.prototype.getSigopsCost = function getSigopsCost(view, flags) {
  ...
  if (this.isCoinbase()) return cost;

  if (flags & Script.flags.VERIFY_P2SH) cost += this.getScripthashSigops(view) * scale;

  if ((flags & Script.flags.VERIFY_WITNESS) === 0) return cost;
  ...
  for (i = 0; i < this.inputs.length; i++) {
    ...
    if (prev.isWitnessScripthash()) {
      ...
      for (j = 0; j < witness.items.length - 1; j++) {
        if (witness.items[j].length > policy.MAX_P2WSH_PUSH) return false;
      }

      redeem = witness.items[witness.items.length - 1];

      if (redeem.length > policy.MAX_P2WSH_SIZE) return false;
      ...
      if (prev.isPubkeyhash()) {
        if (input.witness.length - 1 !== 2) return false;
      ...
      if (prev.isMultisig()) {
        ...
        for (j = 1; j < witness.items.length - 1; j++) {
          if (witness.items[j].length > 73) return false;
        }
      }
...
TX.prototype.getChainValue = function getChainValue(view) {
  ...
  if (this.isCoinbase()) return value;
...
TX.prototype.isWatched = function isWatched(filter) {
  ...
  for (i = 0; i < this.inputs.length; i++) {
    ...
    // Test the witness
    if (input.witness.test(filter)) return true;
  }
...
TX.prototype.fromReader = function fromReader(br) {
  ...
  if (TX.isWitness(br)) return this.fromWitnessReader(br);
...
TX.prototype.fromWitnessReader = function fromWitnessReader(br) {
  ...
  if (flag !== 0) throw new Error('Unknown witness flag.');
...
TX.isTX = function isTX(obj) {
  return obj && Array.isArray(obj.inputs) && typeof obj.locktime === 'number' && typeof obj.witnessHash === 'function';
};
