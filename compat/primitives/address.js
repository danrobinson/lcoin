...
function Address(options) {
  if (!(this instanceof Address)) return new Address(options);
...
Address.prototype.fromOptions = function fromOptions(options) {
  ...
  return this.fromHash(options.hash, options.type, options.version, options.network);
};
...
Address.prototype.isNull = function isNull() {
  ...
  if (this.hash.length === 20) return this.hash.equals(encoding.ZERO_HASH160);

  if (this.hash.length === 32) return this.hash.equals(encoding.ZERO_HASH);
...
Address.prototype.inspect = function inspect() {
  return '<Address:' + ' type=' + this.getType() + ' version=' + this.version + ' base58=' + this.toString() + '>';
};
...
Address.prototype.fromRaw = function fromRaw(data, network) {
  ...
  if (data.length > 40) throw new Error('Address is too long.');
...
Address.getHash = function getHash(data, enc, network) {
  ...
  if (typeof data === 'string') {
    ...
    if (network) {
      network = Network.get(network);
      if (data.network !== network) throw new Error('Network mismatch for address.');
    }
  } else {
