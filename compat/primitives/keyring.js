...
function KeyRing(options, network) {
  if (!(this instanceof KeyRing)) return new KeyRing(options, network);
...
KeyRing.prototype.fromOptions = function fromOptions(options, network) {
  ...
  if (!network) network = options.network;
  ...
  if (script) return this.fromScript(key, script, compressed, network);
...
KeyRing.prototype.fromKey = function fromKey(key, compressed, network) {
  ...
  if (key.length === 32) return this.fromPrivate(key, compressed !== false, network);
...
KeyRing.prototype.getPrivateKey = function getPrivateKey(enc) {
  ...
  if (enc === 'base58') return this.toSecret();
...
KeyRing.prototype.getNestedAddress = function getNestedAddress(enc) {
  ...
  if (enc === 'base58') return this._nestedAddress.toBase58();

  if (enc === 'string') return this._nestedAddress.toString();
...
KeyRing.prototype.getScriptHash256 = function getScriptHash256(enc) {
  ...
  return enc === 'hex' ? this._scriptHash256.toString('hex') : this._scriptHash256;
};
...
KeyRing.prototype.getScriptAddress = function getScriptAddress(enc) {
  ...
  if (enc === 'base58') return this._scriptAddress.toBase58();

  if (enc === 'string') return this._scriptAddress.toString();
...
KeyRing.prototype.getKeyHash = function getKeyHash(enc) {
  if (!this._keyHash) this._keyHash = crypto.hash160(this.publicKey);
...
KeyRing.prototype.getKeyAddress = function getKeyAddress(enc) {
  ...
  if (!this._keyAddress) {
    ...
    if (this.witness) address = Address.fromWitnessPubkeyhash(hash, this.network);else address = Address.fromPubkeyhash(hash, this.network);
    this._keyAddress = address;
  }

  if (enc === 'base58') return this._keyAddress.toBase58();

  if (enc === 'string') return this._keyAddress.toString();
...
KeyRing.prototype.getAddress = function getAddress(enc) {
  if (this.nested) return this.getNestedAddress(enc);
  if (this.script) return this.getScriptAddress(enc);
...
KeyRing.isKeyRing = function isKeyRing(obj) {
  return obj && Buffer.isBuffer(obj.publicKey) && typeof obj.toSecret === 'function';
};
