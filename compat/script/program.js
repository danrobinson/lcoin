...
function Program(version, data) {
  if (!(this instanceof Program)) return new Program(version, data);
...
Program.prototype.getType = function getType() {
  ...
  if (this.version === 1) {
    if (this.data.length === 32) return scriptTypes.WITNESSMASTHASH;
...
Program.prototype.inspect = function inspect() {
  return '<Program:' + ' version=' + this.version + ' data=' + this.data.toString('hex') + ' type=' + common.typesByVal[this.getType()].toLowerCase() + '>';
};
