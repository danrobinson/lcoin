...
function Script(options) {
  if (!(this instanceof Script)) return new Script(options);
...
Script.prototype.fromArray = function fromArray(code) {
  ...
  if (code[0] instanceof Opcode) return this.fromCode(code);
...
Script.prototype.getSubscript = function getSubscript(lastSep) {
  ...
  if (lastSep === 0) return this.clone();
...
Script.prototype.removeSeparators = function removeSeparators() {
  ...
  for (i = 0; i < this.code.length; i++) {
    ...
    if (op.value !== opcodes.OP_CODESEPARATOR) code.push(op);
  }
...
Script.prototype.execute = function execute(stack, flags, tx, index, value, version) {
  ...
  for (ip = 0; ip < this.code.length; ip++) {
    ...
    if (op.data) {
      if (op.data.length > consensus.MAX_SCRIPT_PUSH) throw new ScriptError('PUSH_SIZE', op, ip);
    ...
    if (negate !== 0 && !op.isBranch()) continue;
      ...
      case opcodes.OP_CHECKLOCKTIMEVERIFY:
        {
          ...
          if (locktime.cmpn(0) < 0) throw new ScriptError('NEGATIVE_LOCKTIME', op, ip);
      ...
      case opcodes.OP_NOTIF:
        {
          ...
          if (negate === 0) {
            ...
            if (version === 1 && flags & Script.flags.VERIFY_MINIMALIF) {
              if (val.length > 1) throw new ScriptError('MINIMALIF');

              if (val.length === 1 && val[0] !== 1) throw new ScriptError('MINIMALIF');
            }
      ...
      case opcodes.OP_ELSE:
        {
          ...
          if (!state[state.length - 1]) negate++;else negate--;
      ...
      case opcodes.OP_VERIFY:
        {
          ...
          if (!Script.bool(stack.top(-1))) throw new ScriptError('VERIFY', op, ip);
      ...
      case opcodes.OP_IFDUP:
        {
          ...
          if (Script.bool(val)) stack.push(val);
          break;
        }
      ...
      case opcodes.OP_MAX:
        {
          ...
          if (op.value === opcodes.OP_NUMEQUALVERIFY) {
            if (!Script.bool(stack.top(-1))) throw new ScriptError('NUMEQUALVERIFY', op, ip);
            stack.pop();
          }
      ...
      case opcodes.OP_CHECKSIGVERIFY:
        {
          ...
          if (version === 0) subscript.removeData(sig);
      ...
      case opcodes.OP_CHECKMULTISIGVERIFY:
        {
          ...
          while (i-- > 1) {
            if (!res && flags & Script.flags.VERIFY_NULLFAIL) {
              if (ikey2 === 0 && stack.top(-1).length !== 0) throw new ScriptError('NULLFAIL', op, ip);
            }
            if (ikey2 > 0) ikey2--;
            stack.pop();
          }
          ...
          if (flags & Script.flags.VERIFY_NULLDUMMY) {
            if (stack.top(-1).length !== 0) throw new ScriptError('SIG_NULLDUMMY', op, ip);
          }
...
Script.prototype.removeData = function removeData(data) {
  ...
  for (i = 0; i < this.code.length; i++) {
    ...
    if (op.value === -1) {
      // Can't reserialize
      // a parse error.
      if (index.length > 0) index.push(i);
      break;
    }

    if (!op.data) continue;

    if (!op.isMinimal()) continue;

    if (op.data.equals(data)) index.push(i);
  }
...
Script.prototype.isCode = function isCode() {
  ...
  for (i = 0; i < this.code.length; i++) {
    ...
    if (op.value > opcodes.OP_NOP10) return false;
  }
...
Script.prototype.getType = function getType() {
  ...
  if (this.isWitnessMasthash()) return scriptTypes.WITNESSMASTHASH;
...
Script.prototype.isPubkey = function isPubkey(minimal) {
  ...
  return this.code.length === 2 && common.isKey(this.code[0].data) && this.code[1].value === opcodes.OP_CHECKSIG;
};
...
Script.prototype.isMultisig = function isMultisig(minimal) {
  ...
  if (this.raw.length < 41) return false;

  if (this.raw[this.raw.length - 1] !== opcodes.OP_CHECKMULTISIG) return false;
  ...
  if (n + 3 !== this.code.length) return false;

  for (i = 1; i < n + 1; i++) {
    op = this.code[i];

    if (!common.isKey(op.data)) return false;

    if (minimal) {
      if (!op.isMinimal()) return false;
    }
  }
...
Script.prototype.isNulldata = function isNulldata(minimal) {
  ...
  if (this.raw[0] !== opcodes.OP_RETURN) return false;

  if (this.raw.length === 1) return true;

  if (minimal) {
    if (this.raw.length > policy.MAX_OP_RETURN_BYTES) return false;

    if (this.raw.length === 2) return common.getSmall(this.raw[1]) !== -1;

    if (this.raw[1] >= 0x01 && this.raw[1] <= 0x4b) return this.raw[1] + 2 === this.raw.length;

    if (this.raw[1] === opcodes.OP_PUSHDATA1) return this.raw[2] > 75 && this.raw[2] + 3 === this.raw.length;
...
Script.prototype.forWitness = function () {
  ...
  if (this.isPubkeyhash()) return Script.fromProgram(0, this.get(2));
...
Script.prototype.isWitnessMasthash = function isWitnessMasthash() {
  return this.raw.length === 34 && this.raw[0] === opcodes.OP_1 && this.raw[1] === 0x20;
};
...
Script.prototype.isPubkeyInput = function isPubkeyInput() {
  if (this.raw.length < 10) return false;

  if (this.raw.length > 78) return false;

  if (this.raw[0] > opcodes.OP_PUSHDATA4) return false;
...
Script.prototype.isPubkeyhashInput = function isPubkeyhashInput() {
  if (this.raw.length < 44) return false;

  if (this.raw.length > 148) return false;

  if (this.raw[0] > opcodes.OP_PUSHDATA4) return false;

  return this.code.length === 2 && common.isSignature(this.code[0].data) && common.isKey(this.code[1].data);
};
...
Script.prototype.isMultisigInput = function isMultisigInput() {
  ...
  if (this.raw.length < 20) return false;

  if (this.raw[0] !== opcodes.OP_0) return false;

  if (this.raw[1] > opcodes.OP_PUSHDATA4) return false;

  // We need to rule out scripthash
  // because it may look like multisig.
  if (this.isScripthashInput()) return false;

  if (this.code.length < 3) return false;

  for (i = 1; i < this.code.length; i++) {
    if (!common.isSignature(this.code[i].data)) return false;
  }
...
Script.prototype.isScripthashInput = function isScripthashInput() {
  ...
  if (this.raw.length < 2) return false;
  ...
  // opcodes.
  if (op.data.length === 0) return false;

  if (common.isSignatureEncoding(op.data)) return false;

  if (common.isKeyEncoding(op.data)) return false;
...
Script.getCoinbaseHeight = function getCoinbaseHeight(raw) {
  ...
  if (raw.length < 1 + raw[0]) return -1;
  ...
  if (!data.equals(op.data)) return -1;
...
Script.prototype.getNumber = function getNumber(i) {
  ...
  if (small !== -1) return new BN(small);

  if (!op || !op.data || op.data.length > 5) return null;
...
Script.prototype.getScripthashSigops = function getScripthashSigops(input) {
  ...
  if (input.code.length === 0) return 0;
...
Script.prototype.getWitnessSigops = function getWitnessSigops(input, witness) {
  ...
  if (this.isProgram()) return Script.witnessSigops(this.toProgram(), witness);
  ...
  if (this.isScripthash() && input.isPushOnly()) {
    ...
    if (redeem && redeem.isProgram()) return Script.witnessSigops(redeem.toProgram(), witness);
  }
...
Script.prototype.fromString = function fromString(code) {
  ...
  for (i = 0; i < code.length; i++) {
    ...
    if (!util.isUpperCase(symbol)) symbol = symbol.toUpperCase();

    if (!util.startsWith(symbol, 'OP_')) symbol = 'OP_' + symbol;
...
Script.verify = function verify(input, witness, output, tx, i, value, flags) {
  ...
  if (stack.length === 0 || !Script.bool(stack.top(-1))) throw new ScriptError('EVAL_FALSE');
...
Script.verifyProgram = function verifyProgram(witness, output, flags, tx, i, value) {
  ...
  if (program.version === 0) {
    if (program.data.length === 32) {
      ...
      if (!crypto.sha256(witnessScript).equals(program.data)) throw new ScriptError('WITNESS_PROGRAM_MISMATCH');
  ...
  // Verify the stack values.
  if (stack.length !== 1 || !Script.bool(stack.top(-1))) throw new ScriptError('EVAL_FALSE');
...
Script.verifyMast = function verifyMast(program, stack, output, flags, tx, i, value) {
  ...
  if (stack.length < 4) throw new ScriptError('INVALID_MAST_STACK');

  metadata = stack.top(-1);
  if (metadata.length < 1 || metadata.length > 5) throw new ScriptError('INVALID_MAST_STACK');

  subscripts = metadata[0];
  if (subscripts === 0 || stack.length < subscripts + 3) throw new ScriptError('INVALID_MAST_STACK');

  ops = subscripts;
  scriptRoot.writeU8(subscripts);

  if (metadata[metadata.length - 1] === 0x00) throw new ScriptError('INVALID_MAST_STACK');

  for (j = 1; j < metadata.length; j++) {
    version |= metadata[i] << 8 * (j - 1);
  }if (version < 0) version += 0x100000000;

  if (version > 0) {
    if (flags & Script.flags.DISCOURAGE_UPGRADABLE_WITNESS_PROGRAM) throw new ScriptError('DISCOURAGE_UPGRADABLE_WITNESS_PROGRAM');
  }
  ...
  if (pathdata.length & 0x1f) throw new ScriptError('INVALID_MAST_STACK');

  depth = pathdata.length >>> 5;

  if (depth > 32) throw new ScriptError('INVALID_MAST_STACK');

  ops += depth;
  if (version === 0) {
    if (ops > consensus.MAX_SCRIPT_OPS) throw new ScriptError('OP_COUNT');
  }
  ...
  if (posdata.length > 4) throw new ScriptError('INVALID_MAST_STACK');

  pos = 0;
  if (posdata.length > 0) {
    if (posdata[posdata.length - 1] === 0x00) throw new ScriptError('INVALID_MAST_STACK');
  ...
  if (depth < 32) {
    if (pos >= 1 << depth >>> 0) throw new ScriptError('INVALID_MAST_STACK');
  }
  ...
  for (j = 0; j < subscripts; j++) {
    ...
    if (version === 0) {
      if (scripts.written + script.length > consensus.MAX_SCRIPT_SIZE) throw new ScriptError('SCRIPT_SIZE');
    }
  ...
  if (!mastRoot.equals(program.data)) throw new ScriptError('WITNESS_PROGRAM_MISMATCH');
...
Script.isScript = function isScript(obj) {
  return obj && Buffer.isBuffer(obj.raw) && typeof obj.getSubscript === 'function';
};
