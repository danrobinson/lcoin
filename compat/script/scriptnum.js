...
function ScriptNum(value) {
  if (!(this instanceof ScriptNum)) return new ScriptNum(value);
...
ScriptNum.prototype.toString = function toString(base) {
  ...
  if (base === 10 || base === 'dec') return this.value.toString(10);
...
ScriptNum.prototype.fromString = function fromString(str, base) {
  ...
  if (base === 10 || base === 'dec') {
    for (i = 0; i < str.length; i++) {
      ...
      if (nonzero === 0 && ch === '0') continue;

      if (!(ch >= '0' && ch <= '9')) throw new Error('Parse error.');
    ...
    if (negative) this.value = -this.value;
...
ScriptNum.prototype.fromRaw = function fromRaw(data, minimal, limit) {
  ...
  if (data.length > limit) throw new ScriptError('UNKNOWN_ERROR', 'Script number overflow.');
  ...
  if (minimal) {
    if ((data[data.length - 1] & 0x7f) === 0) {
      if (data.length === 1 || !(data[data.length - 2] & 0x80)) {
        throw new ScriptError('UNKNOWN_ERROR', 'Non-minimally encoded Script number.');
      }
    }
  }
...
ScriptNum.prototype.toRaw = function toRaw() {
  ...
  // Append sign bit.
  if (data[size - 1] & 0x80) data[size] = negative ? 0x80 : 0;else if (negative) data[size - 1] |= 0x80;
