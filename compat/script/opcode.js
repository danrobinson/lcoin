...
function Opcode(value, data) {
  if (!(this instanceof Opcode)) return new Opcode(value, data);
...
Opcode.prototype.isMinimal = function isMinimal() {
  ...
  if (this.data.length === 0) return this.value === opcodes.OP_0;

  if (this.data.length === 1 && this.data[0] >= 1 && this.data[0] <= 16) return false;

  if (this.data.length === 1 && this.data[0] === 0x81) return false;

  if (this.data.length <= 75) return this.value === this.data.length;

  if (this.data.length <= 255) return this.value === opcodes.OP_PUSHDATA1;

  if (this.data.length <= 65535) return this.value === opcodes.OP_PUSHDATA2;
...
Opcode.prototype.getSize = function getSize() {
  ...
  if (this.value <= 0x4b) return 1 + this.data.length;
...
Opcode.fromData = function fromData(data) {
  if (data.length === 0) return Opcode.fromOp(opcodes.OP_0);

  if (data.length === 1) {
    if (data[0] >= 1 && data[0] <= 16) return Opcode.fromOp(data[0] + 0x50);

    if (data[0] === 0x81) return Opcode.fromOp(opcodes.OP_1NEGATE);
  }
...
Opcode.fromPush = function fromPush(data) {
  ...
  if (data.length <= 0xffffffff) return new Opcode(opcodes.OP_PUSHDATA4, data);
...
Opcode.from = function from(data) {
  if (data instanceof Opcode) return data;

  if (typeof data === 'number') return Opcode.fromOp(data);

  if (Buffer.isBuffer(data)) return Opcode.fromData(data);

  if (typeof data === 'string') return Opcode.fromString(data, 'utf8');

  if (BN.isBN(data)) return Opcode.fromNumber(data);
...
Opcode.fromSymbol = function fromSymbol(name) {
  ...
  if (!util.isUpperCase(name)) name = name.toUpperCase();

  if (!util.startsWith(name, 'OP_')) name = 'OP_' + name;
...
Opcode.prototype.toSymbol = function toSymbol() {
  ...
  if (symbol == null) symbol = util.hex8(op);
...
Opcode.isOpcode = function isOpcode(obj) {
  return obj && typeof obj.value === 'number' && (Buffer.isBuffer(obj.data) || obj.data === null);
};
