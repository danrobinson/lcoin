...
exports.getSmall = function getSmall(op) {
  ...
  if (op === exports.opcodes.OP_0) return 0;

  if (op >= exports.opcodes.OP_1 && op <= exports.opcodes.OP_16) return op - 0x50;
...
exports.formatASM = function formatASM(code, decode) {
  ...
  if (code.length > 0 && code[0].value === exports.opcodes.OP_RETURN) decode = false;
...
exports.num = function num(value, minimal, size) {
  ...
  if (value.length > size) throw new exports.ScriptError('UNKNOWN_ERROR', 'Script number overflow.');
  ...
  if (minimal && value.length > 0) {
    ...
    if (!(value[value.length - 1] & 0x7f)) {
      if (value.length === 1 || !(value[value.length - 2] & 0x80)) {
        throw new exports.ScriptError('UNKNOWN_ERROR', 'Non-minimally encoded Script number.');
      }
    }
  }

  if (value.length === 0) return new BN(0);
  ...
  if (value[value.length - 1] & 0x80) result.setn(value.length * 8 - 1, 0).ineg();
...
exports.array = function (value) {
  ...
  if (util.isNumber(value)) value = new BN(value);

  assert(BN.isBN(value));

  if (value.cmpn(0) === 0) return exports.STACK_FALSE;
  ...
  if (result[result.length - 1] & 0x80) result.push(neg ? 0x80 : 0);else if (neg) result[result.length - 1] |= 0x80;
...
exports.ScriptError = function ScriptError(code, op, ip) {
  if (!(this instanceof ScriptError)) return new ScriptError(code, op, ip);
