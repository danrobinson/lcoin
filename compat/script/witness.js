...
Witness.prototype.test = function test(filter) {
  ...
  for (i = 0; i < this.items.length; i++) {
    ...
    if (filter.test(chunk)) return true;
  }
...
Witness.prototype.getSmall = function getSmall(i) {
  ...
  if (!item || item.length > 1) return -1;
  if (item.length === 0) return 0;
  if (!(item[0] >= 1 && item[1] <= 16)) return -1;
  return item[0];
};
...
Witness.encodeItem = function encodeItem(data) {
  if (data instanceof Opcode) data = data.data || data.value;

  if (typeof data === 'number') {
    if (data === opcodes.OP_1NEGATE) return STACK_NEGATE;

    if (data === opcodes.OP_0) return STACK_FALSE;

    if (data >= opcodes.OP_1 && data <= opcodes.OP_16) return Buffer.from([data - 0x50]);

    throw new Error('Non-push opcode in witness.');
  }

  if (BN.isBN(data)) return common.array(data);
...
Witness.isWitness = function isWitness(obj) {
  return obj && Array.isArray(obj.items) && typeof obj.toStack === 'function';
};
