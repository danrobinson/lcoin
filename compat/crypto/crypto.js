...
crypto.createMerkleTree = function createMerkleTree(leaves) {
  ...
  for (j = 0; size > 1; size = (size + 1) / 2 | 0) {
    for (i = 0; i < size; i += 2) {
      ...
      if (k === i + 1 && k + 1 === size && left.equals(right)) {
        malleated = true;
      }
...
if (native) crypto.createMerkleTree = native.createMerkleTree;
...
if (native) crypto.verifyMerkleBranch = native.verifyMerkleBranch;
...
if (native) crypto.cleanse = native.cleanse;
