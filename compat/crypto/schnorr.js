...
schnorr._sign = function _sign(msg, prv, k, hash, pubnonce) {
  ...
  if (k.cmpn(0) === 0) return;

  if (k.cmp(curve.n) >= 0) return;
  ...
  if (h.cmpn(0) === 0) return;

  if (h.cmp(curve.n) >= 0) return;

  s = h.imul(prv);
  s = k.isub(s);
  s = s.umod(curve.n);

  if (s.cmpn(0) === 0) return;
...
schnorr.generateNoncePair = function generateNoncePair(msg, priv, data, ncb) {
  ...
  if (k.cmpn(0) === 0) throw new Error('Bad nonce.');

  if (k.cmp(curve.n) >= 0) throw new Error('Bad nonce.');
