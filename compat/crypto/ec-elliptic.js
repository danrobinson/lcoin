...
function normalizeLength(sig) {
  ...
  if (data[p.place++] !== 0x30) return sig;

  len = getLength(data, p);

  if (data.length > len + p.place) data = data.slice(0, len + p.place);

  if (data[p.place++] !== 0x02) return sig;
  ...
  if (data.length > slen + p.place) data = data.slice(0, slen + p.place);
