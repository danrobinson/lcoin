...
function AESKey(key, bits) {
  if (!(this instanceof AESKey)) return new AESKey(key, bits);
...
AESKey.prototype.getEncryptKey = function getEncryptKey() {
  ...
  if (this.bits === 128) {
    for (;;) {
      ...
      key[kp + 4] = key[kp + 0] ^ TE2[tmp >>> 16 & 0xff] & 0xff000000 ^ TE3[tmp >>> 8 & 0xff] & 0x00ff0000 ^ TE0[tmp >>> 0 & 0xff] & 0x0000ff00 ^ TE1[tmp >>> 24 & 0xff] & 0x000000ff ^ RCON[i];
  ...
  if (this.bits === 192) {
    for (;;) {
      ...
      key[kp + 6] = key[kp + 0] ^ TE2[tmp >>> 16 & 0xff] & 0xff000000 ^ TE3[tmp >>> 8 & 0xff] & 0x00ff0000 ^ TE0[tmp >>> 0 & 0xff] & 0x0000ff00 ^ TE1[tmp >>> 24 & 0xff] & 0x000000ff ^ RCON[i];
  ...
  if (this.bits === 256) {
    for (;;) {
      ...
      key[kp + 8] = key[kp + 0] ^ TE2[tmp >>> 16 & 0xff] & 0xff000000 ^ TE3[tmp >>> 8 & 0xff] & 0x00ff0000 ^ TE0[tmp >>> 0 & 0xff] & 0x0000ff00 ^ TE1[tmp >>> 24 & 0xff] & 0x000000ff ^ RCON[i];
      ...
      key[kp + 12] = key[kp + 4] ^ TE2[tmp >>> 24 & 0xff] & 0xff000000 ^ TE3[tmp >>> 16 & 0xff] & 0x00ff0000 ^ TE0[tmp >>> 8 & 0xff] & 0x0000ff00 ^ TE1[tmp >>> 0 & 0xff] & 0x000000ff;
...
AESKey.prototype.getDecryptKey = function getDecryptKey() {
  ...
  if (this.decryptKey) return this.decryptKey;
  ...
  for (i = 1; i < this.rounds; i++) {
    kp += 4;
    key[kp + 0] = TD0[TE1[key[kp + 0] >>> 24 & 0xff] & 0xff] ^ TD1[TE1[key[kp + 0] >>> 16 & 0xff] & 0xff] ^ TD2[TE1[key[kp + 0] >>> 8 & 0xff] & 0xff] ^ TD3[TE1[key[kp + 0] >>> 0 & 0xff] & 0xff];
    key[kp + 1] = TD0[TE1[key[kp + 1] >>> 24 & 0xff] & 0xff] ^ TD1[TE1[key[kp + 1] >>> 16 & 0xff] & 0xff] ^ TD2[TE1[key[kp + 1] >>> 8 & 0xff] & 0xff] ^ TD3[TE1[key[kp + 1] >>> 0 & 0xff] & 0xff];
    key[kp + 2] = TD0[TE1[key[kp + 2] >>> 24 & 0xff] & 0xff] ^ TD1[TE1[key[kp + 2] >>> 16 & 0xff] & 0xff] ^ TD2[TE1[key[kp + 2] >>> 8 & 0xff] & 0xff] ^ TD3[TE1[key[kp + 2] >>> 0 & 0xff] & 0xff];
    key[kp + 3] = TD0[TE1[key[kp + 3] >>> 24 & 0xff] & 0xff] ^ TD1[TE1[key[kp + 3] >>> 16 & 0xff] & 0xff] ^ TD2[TE1[key[kp + 3] >>> 8 & 0xff] & 0xff] ^ TD3[TE1[key[kp + 3] >>> 0 & 0xff] & 0xff];
  }
...
AESKey.prototype.encryptBlock = function encryptBlock(input) {
  ...
  for (;;) {
    t0 = TE0[s0 >>> 24 & 0xff] ^ TE1[s1 >>> 16 & 0xff] ^ TE2[s2 >>> 8 & 0xff] ^ TE3[s3 >>> 0 & 0xff] ^ key[kp + 4];
    t1 = TE0[s1 >>> 24 & 0xff] ^ TE1[s2 >>> 16 & 0xff] ^ TE2[s3 >>> 8 & 0xff] ^ TE3[s0 >>> 0 & 0xff] ^ key[kp + 5];
    t2 = TE0[s2 >>> 24 & 0xff] ^ TE1[s3 >>> 16 & 0xff] ^ TE2[s0 >>> 8 & 0xff] ^ TE3[s1 >>> 0 & 0xff] ^ key[kp + 6];
    t3 = TE0[s3 >>> 24 & 0xff] ^ TE1[s0 >>> 16 & 0xff] ^ TE2[s1 >>> 8 & 0xff] ^ TE3[s2 >>> 0 & 0xff] ^ key[kp + 7];

    kp += 8;

    if (--r === 0) break;

    s0 = TE0[t0 >>> 24 & 0xff] ^ TE1[t1 >>> 16 & 0xff] ^ TE2[t2 >>> 8 & 0xff] ^ TE3[t3 >>> 0 & 0xff] ^ key[kp + 0];
    s1 = TE0[t1 >>> 24 & 0xff] ^ TE1[t2 >>> 16 & 0xff] ^ TE2[t3 >>> 8 & 0xff] ^ TE3[t0 >>> 0 & 0xff] ^ key[kp + 1];
    s2 = TE0[t2 >>> 24 & 0xff] ^ TE1[t3 >>> 16 & 0xff] ^ TE2[t0 >>> 8 & 0xff] ^ TE3[t1 >>> 0 & 0xff] ^ key[kp + 2];
    s3 = TE0[t3 >>> 24 & 0xff] ^ TE1[t0 >>> 16 & 0xff] ^ TE2[t1 >>> 8 & 0xff] ^ TE3[t2 >>> 0 & 0xff] ^ key[kp + 3];
  }

  // Apply last round and map cipher
  // state to byte array block.
  s0 = TE2[t0 >>> 24 & 0xff] & 0xff000000 ^ TE3[t1 >>> 16 & 0xff] & 0x00ff0000 ^ TE0[t2 >>> 8 & 0xff] & 0x0000ff00 ^ TE1[t3 >>> 0 & 0xff] & 0x000000ff ^ key[kp + 0];
  s1 = TE2[t1 >>> 24 & 0xff] & 0xff000000 ^ TE3[t2 >>> 16 & 0xff] & 0x00ff0000 ^ TE0[t3 >>> 8 & 0xff] & 0x0000ff00 ^ TE1[t0 >>> 0 & 0xff] & 0x000000ff ^ key[kp + 1];
  s2 = TE2[t2 >>> 24 & 0xff] & 0xff000000 ^ TE3[t3 >>> 16 & 0xff] & 0x00ff0000 ^ TE0[t0 >>> 8 & 0xff] & 0x0000ff00 ^ TE1[t1 >>> 0 & 0xff] & 0x000000ff ^ key[kp + 2];
  s3 = TE2[t3 >>> 24 & 0xff] & 0xff000000 ^ TE3[t0 >>> 16 & 0xff] & 0x00ff0000 ^ TE0[t1 >>> 8 & 0xff] & 0x0000ff00 ^ TE1[t2 >>> 0 & 0xff] & 0x000000ff ^ key[kp + 3];
...
AESKey.prototype.decryptBlock = function decryptBlock(input) {
  ...
  for (;;) {
    t0 = TD0[s0 >>> 24 & 0xff] ^ TD1[s3 >>> 16 & 0xff] ^ TD2[s2 >>> 8 & 0xff] ^ TD3[s1 >>> 0 & 0xff] ^ key[kp + 4];
    t1 = TD0[s1 >>> 24 & 0xff] ^ TD1[s0 >>> 16 & 0xff] ^ TD2[s3 >>> 8 & 0xff] ^ TD3[s2 >>> 0 & 0xff] ^ key[kp + 5];
    t2 = TD0[s2 >>> 24 & 0xff] ^ TD1[s1 >>> 16 & 0xff] ^ TD2[s0 >>> 8 & 0xff] ^ TD3[s3 >>> 0 & 0xff] ^ key[kp + 6];
    t3 = TD0[s3 >>> 24 & 0xff] ^ TD1[s2 >>> 16 & 0xff] ^ TD2[s1 >>> 8 & 0xff] ^ TD3[s0 >>> 0 & 0xff] ^ key[kp + 7];

    kp += 8;

    if (--r === 0) break;

    s0 = TD0[t0 >>> 24 & 0xff] ^ TD1[t3 >>> 16 & 0xff] ^ TD2[t2 >>> 8 & 0xff] ^ TD3[t1 >>> 0 & 0xff] ^ key[kp + 0];
    s1 = TD0[t1 >>> 24 & 0xff] ^ TD1[t0 >>> 16 & 0xff] ^ TD2[t3 >>> 8 & 0xff] ^ TD3[t2 >>> 0 & 0xff] ^ key[kp + 1];
    s2 = TD0[t2 >>> 24 & 0xff] ^ TD1[t1 >>> 16 & 0xff] ^ TD2[t0 >>> 8 & 0xff] ^ TD3[t3 >>> 0 & 0xff] ^ key[kp + 2];
    s3 = TD0[t3 >>> 24 & 0xff] ^ TD1[t2 >>> 16 & 0xff] ^ TD2[t1 >>> 8 & 0xff] ^ TD3[t0 >>> 0 & 0xff] ^ key[kp + 3];
  }

  // Apply last round and map cipher
  // state to byte array block.
  s0 = TD4[t0 >>> 24 & 0xff] << 24 ^ TD4[t3 >>> 16 & 0xff] << 16 ^ TD4[t2 >>> 8 & 0xff] << 8 ^ TD4[t1 >>> 0 & 0xff] << 0 ^ key[kp + 0];
  s1 = TD4[t1 >>> 24 & 0xff] << 24 ^ TD4[t0 >>> 16 & 0xff] << 16 ^ TD4[t3 >>> 8 & 0xff] << 8 ^ TD4[t2 >>> 0 & 0xff] << 0 ^ key[kp + 1];
  s2 = TD4[t2 >>> 24 & 0xff] << 24 ^ TD4[t1 >>> 16 & 0xff] << 16 ^ TD4[t0 >>> 8 & 0xff] << 8 ^ TD4[t3 >>> 0 & 0xff] << 0 ^ key[kp + 2];
  s3 = TD4[t3 >>> 24 & 0xff] << 24 ^ TD4[t2 >>> 16 & 0xff] << 16 ^ TD4[t1 >>> 8 & 0xff] << 8 ^ TD4[t0 >>> 0 & 0xff] << 0 ^ key[kp + 3];
...
function AESCipher(key, iv, bits, mode) {
  if (!(this instanceof AESCipher)) return new AESCipher(key, iv, mode);
...
AESCipher.prototype.update = function update(data) {
  ...
  for (i = 0; i < len; i += 16) {
    ...
    if (this.mode === 'cbc') block = xor(block, this.prev);
    this.prev = this.key.encryptBlock(block);
    blocks.push(this.prev);
  }

  if (trailing > 0) this.waiting = data.slice(len);
...
function AESDecipher(key, iv, bits, mode) {
  if (!(this instanceof AESDecipher)) return new AESDecipher(key, iv, mode);
...
AESDecipher.prototype.update = function update(data) {
  ...
  for (i = 0; i < len; i += 16) {
    ...
    if (this.mode === 'cbc') block = xor(block, chunk);
    blocks.push(block);
  }
...
AESDecipher.prototype.final = function final() {
  ...
  if (n === 0 || n > b) throw new Error('Bad decrypt (padding).');

  for (i = 0; i < n; i++) {
    if (block[--b] !== n) throw new Error('Bad decrypt (padding).');
  }

  // Slice off the padding unless
  // the entire block was padding.
  if (n === 16) return Buffer.alloc(0);
...
var TD4 = [];

var RCON = [0x01000000, 0x02000000, 0x04000000, 0x08000000, 0x10000000, 0x20000000, 0x40000000, 0x80000000, 0x1B000000, 0x36000000];